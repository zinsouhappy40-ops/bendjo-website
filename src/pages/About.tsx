...
function About() {
  return (
    <>
      <DocumentMeta
        title="La provenance vivante | BenDjo"
        description="Entrez dans l’univers BenDjo : des ingrédients locaux, des infusions naturelles fabriquées au Bénin et des moments à partager."
      />
      ...
      <div>
        ...
        <section className="bg-ink py-10 text-cream sm:py-14 lg:py-18" aria-labelledby="purpose-title">
          <Container>
            <div className="grid gap-6 border-t border-cream/25 pt-5 md:grid-cols-[1.1fr_0.9fr] md:gap-10 lg:gap-14">
              <div>
                <p className="type-kicker max-w-xs text-cream/70">Pourquoi valoriser ces infusions</p>
                <h2 id="purpose-title" className="type-section-title-lg mt-4 max-w-[15ch] text-cream">
              ...
              <div className="grid gap-5 border-t border-cream/20 pt-5 sm:grid-cols-2 sm:gap-8 md:mt-8 md:self-end">
                <p className="max-w-md text-base leading-8 text-cream/80">
        ...
        <section id="provenance" className="scroll-mt-20 bg-kraft/20 py-12 sm:py-16 lg:py-20" aria-labelledby="terroir-title">
          <Container>
            <div className="grid gap-5 md:grid-cols-[0.78fr_1.22fr] md:items-end md:gap-10 lg:gap-16">
              <div>
                <div className="flex items-center gap-3">
                  ...
                  <p className="type-label text-ink/70">Terroir béninois</p>
                </div>
            ...
            <div className="mt-8 grid grid-cols-2 gap-3 sm:mt-10 sm:gap-4 lg:grid-cols-[1.2fr_0.8fr] lg:items-stretch">
          ...
          <Container>
            <div className="border-t border-cream/25 pt-4 sm:pt-5">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between sm:gap-8">
                <div>
                  <p className="type-kicker text-cream/70">La philosophie, dans les faits</p>
                  <h2 id="philosophy-title" className="type-section-title mt-2 max-w-[16ch] text-cream">
                  Une identité visible dans l’offre.
                  </h2>
                </div>
                <p className="max-w-md border-l border-kraft pl-4 text-sm leading-7 text-cream/75 sm:mb-1 sm:max-w-xs">
              ...
              <ol data-motion-group className="mt-7 grid border-t border-cream/25 sm:mt-8 sm:grid-cols-2 lg:grid-cols-4" aria-label="Principes de BenDjo">
                {principles.map((principle) => (
                  <li data-motion-item key={principle.title} className="grid gap-2 border-b border-cream/25 py-4 sm:min-h-40 sm:border-r sm:px-5 sm:py-5 first:pl-0 last:border-r-0 last:pr-0 lg:min-h-44 lg:px-6">
                    <h3 className="max-w-[18ch] font-display text-xl leading-tight text-cream sm:text-2xl">{principle.title}</h3>
                    <p className="max-w-[28ch] text-sm leading-6 text-cream/80">{principle.text}</p>
                  </li>
                ))}
              </ol>
            </div>
        ...
        <section className="bg-cream pb-16 sm:pb-20 lg:pb-28" aria-labelledby="products-life-title">
          <Container>
            ...
            <div data-motion-group className="mt-8 grid gap-3 sm:mt-10 md:grid-cols-2 lg:grid-cols-3 lg:gap-4">
              {infusionProfiles.map(({ product, profile, tone, textTone, imagePosition, image, imageSrcSet, imageWidth, imageHeight }, index) => (
                <article data-motion-item key={product.id} className={`overflow-hidden rounded-bendjo-md ${tone} ${textTone} shadow-[0_12px_32px_rgb(75_127_82_/_0.08)] ring-1 ring-inset ring-leaf/10 ${index === 1 ? "md:translate-y-5" : ""}`}>
                  ...
                  <div className="flex min-h-48 flex-col justify-between gap-7 p-5 sm:min-h-52 sm:p-6 lg:p-7">
                    <p className="type-label opacity-75">Infusion naturelle</p>
                    <div>
                      ...
                      <p className="mt-1 text-xs opacity-75">Boîte de 10 sachets</p>
                    </div>
                  </div>
                </article>
              ))}
        ...
        <section className="bg-leaf py-16 text-cream sm:py-20 lg:py-24" aria-labelledby="today-title">
          <Container>
            <div className="grid gap-9 lg:grid-cols-[0.78fr_1.22fr] lg:gap-20">
              <div>
                <p className="type-kicker text-cream">BenDjo aujourd’hui</p>
              ...
              <dl className="grid border-t border-cream/30 sm:grid-cols-2">
                <div className="border-b border-cream/30 py-6 sm:pr-8">
                  <dt className="type-label text-cream">Ancrage</dt>
        ...
        <section className="bg-kraft/20 pb-10 pt-12 sm:pb-12 sm:pt-14 lg:pb-14 lg:pt-16" aria-labelledby="people-title">
          <Container>
            <div className="grid gap-5 border-t border-ink/20 pt-5 md:grid-cols-[0.78fr_1.22fr] md:gap-10 lg:gap-16">
              <div>
                <p className="type-kicker text-ink/70">La marque, ici</p>
          ...
          <Container>
            <div className="grid gap-7 lg:grid-cols-[1.1fr_0.9fr] lg:items-end lg:gap-16">
              <div>
                <div className="flex items-center gap-3">
                  <span className="h-px w-10 bg-kraft" aria-hidden="true" />
                  <p className="type-label text-cream/70">Prolonger l’expérience</p>
                </div>
                <h2 id="about-cta-title" className="type-section-title-lg mt-4 text-cream">Choisir ce que vous souhaitez partager.</h2>
              </div>
              <div className="lg:pb-1">
                <p className="max-w-xl leading-8 text-cream/80">Une infusion pour votre quotidien ou un moment à imaginer avec BenDjo pour votre entreprise.</p>
                <div className="mt-5 flex flex-col gap-3 sm:mt-6 sm:flex-row lg:mt-7">
                  <Link to="/products" className={`${primaryLinkStyles} sm:min-w-52`}>Découvrir les infusions</Link>
