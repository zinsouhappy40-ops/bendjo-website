  ...
  return (
    <>
      ...
      <div>
        ...
        <section className="bg-leaf py-5 text-cream sm:py-6" aria-label="Choisir un parcours de contact">
          <Container className="grid gap-7 md:grid-cols-[1.2fr_0.8fr] md:gap-0">
            <div className="md:border-r md:border-cream/20 md:pr-10 lg:pr-14">
              ...
              <p className="mt-2 font-display text-2xl font-semibold leading-tight text-cream">Petit-déjeuner, traiteur ou lancement de produit.</p>
              <p className="mt-2 max-w-xl text-sm leading-6 text-cream/80">Présentez votre besoin dans le formulaire ci-dessous.</p>
              <a href="#contact-form-title" className="type-action mt-3 inline-flex min-h-11 items-center rounded-bendjo-sm text-cream underline decoration-kraft decoration-2 underline-offset-4 hover:text-cream/85 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cream">Accéder au formulaire</a>
            </div>
            <div className="border-t border-cream/20 pt-7 md:border-t-0 md:pl-10 md:pt-0 lg:pl-14">
              <p className="type-kicker text-cream">Pour une commande personnelle</p>
              <p className="mt-2 max-w-md text-sm leading-6 text-cream/80">Découvrez les infusions, préparez votre sélection, puis poursuivez avec le bouton WhatsApp du panier.</p>
              <Link to="/products" className="type-action mt-3 inline-flex min-h-11 items-center rounded-bendjo-sm text-cream underline decoration-kraft decoration-2 underline-offset-4 hover:text-cream/85 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cream">Choisir une infusion</Link>
            </div>
          </Container>
        </section>
        ...
        <section className="bg-kraft/15 py-8 sm:py-10 lg:py-12" aria-labelledby="contact-form-title">
          <Container className="grid min-w-0 gap-y-8 lg:gap-y-10">
            <div className="min-w-0 lg:max-w-2xl">
              <p className="type-label text-leaf/75">Demande professionnelle</p>
              <h2 id="contact-form-title" className="type-section-title-lg mt-3 scroll-mt-24 text-leaf">Présentez votre besoin.</h2>
              <p className="type-body mt-4 max-w-xl text-pretty text-leaf/80">Renseignez les éléments dont vous disposez. Votre demande sera préparée dans WhatsApp avant que vous ne choisissiez de l’envoyer.</p>
            </div>
            ...
            <div className="min-w-0 max-w-4xl border-t border-leaf/30 pt-5 sm:pt-6">
              <aside aria-labelledby="direct-contact-title">
                ...
                <p className="mt-2 max-w-xl leading-7 text-leaf/80 sm:mt-3">Vous pouvez aussi joindre BenDjo par email ou par téléphone.</p>
                <div className="mt-3 flex flex-col gap-1 text-sm sm:mt-4 sm:flex-row sm:flex-wrap sm:gap-x-6">
                   <a href="mailto:bendjobenin@gmail.com" className="inline-flex min-h-11 w-fit items-center gap-2 rounded-bendjo-sm font-semibold text-leaf underline decoration-kraft decoration-2 underline-offset-4 hover:text-leaf/75 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-leaf"><EnvelopeSimple size={18} weight="regular" aria-hidden="true" />bendjobenin@gmail.com</a>
                   <a href="tel:+2290153148013" className="inline-flex min-h-11 w-fit items-center gap-2 rounded-bendjo-sm font-semibold text-leaf underline decoration-kraft decoration-2 underline-offset-4 hover:text-leaf/75 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-leaf"><Phone size={18} weight="regular" aria-hidden="true" />+229 01 53 14 80 13</a>
                </div>
              </aside>
            </div>
          </Container>
