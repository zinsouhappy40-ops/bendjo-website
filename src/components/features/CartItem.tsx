...
function CartItem({ item, compact = false, onIncrease, onDecrease, onRemove }: CartItemProps) {
  ...
  return (
    <li className={`border-t py-4 first:border-t-0 first:pt-0 ${compact ? "border-cream/15 text-cream sm:py-3" : "border-leaf/15 sm:py-5"}`}>
      <div className="flex gap-4">
        ...
        <div className="min-w-0 flex-1">
            <h3 className={`font-display text-xl ${compact ? "text-cream" : "text-leaf"}`}>{product.name}</h3>
            <p className={`mt-1 text-sm ${compact ? "text-cream/70" : "text-leaf/75"}`}>{product.format}</p>
          <div className="mt-3 flex flex-wrap items-center gap-2">
             <button type="button" className={`group inline-flex min-h-11 min-w-11 items-center justify-center rounded-bendjo-sm transition-colors focus-visible:outline-none focus-visible:ring-2 ${compact ? "text-cream focus-visible:ring-cream" : "text-leaf focus-visible:ring-leaf"}`} onClick={() => onDecrease(product.id)} aria-label={`Diminuer la quantité d’${product.name}`}>
            ...
            <span className={`min-w-8 text-center text-sm ${compact ? "text-cream" : "text-leaf"}`} aria-label={`Quantité : ${quantity}`}>{quantity}</span>
             <button type="button" className={`group inline-flex min-h-11 min-w-11 items-center justify-center rounded-bendjo-sm transition-colors focus-visible:outline-none focus-visible:ring-2 ${compact ? "text-cream focus-visible:ring-cream" : "text-leaf focus-visible:ring-leaf"}`} onClick={() => onIncrease(product.id)} aria-label={`Augmenter la quantité d’${product.name}`}>
               <span className={`inline-flex h-8 w-8 items-center justify-center rounded-bendjo-sm border transition-colors ${compact ? "border-cream/40 group-hover:border-cream/70 group-hover:bg-cream/15" : "border-leaf/20 group-hover:border-leaf/45 group-hover:bg-leaf/5"}`}>
                 <Plus size={16} weight="regular" aria-hidden="true" />
               </span>
             </button>
             <button type="button" className={`inline-flex min-h-11 items-center gap-2 whitespace-nowrap rounded-bendjo-sm px-3 text-sm underline underline-offset-4 transition-colors focus-visible:outline-none focus-visible:ring-2 ${compact ? "text-cream hover:text-cream/75 focus-visible:ring-cream" : "text-leaf hover:text-leaf/70 focus-visible:ring-leaf"}`} onClick={() => onRemove(product.id)} aria-label={`Supprimer ${product.name}`}>
