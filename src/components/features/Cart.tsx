    ...
    return (
      <section id="cart-panel" className="fixed inset-x-3 top-[calc(76px+0.75rem)] z-50 overflow-hidden rounded-bendjo-md border border-ink/15 bg-ink p-4 text-cream shadow-bendjo-cart sm:absolute sm:inset-x-auto sm:right-0 sm:top-[calc(100%+0.75rem)] sm:w-[23rem] sm:p-5" role="dialog" aria-modal="false" aria-labelledby="cart-panel-title">
        ...
        {state.items.length === 0 ? (
          <div className="py-6">
            <p className="font-display text-xl text-cream">Votre sélection est vide.</p>
            <p className="mt-2 text-sm leading-6 text-cream/70">Ajoutez une infusion pour préparer votre commande.</p>
          </div>
        ) : (
          <>
            <div className="flex items-center justify-between gap-3 pt-3">
              <p className="text-xs font-semibold uppercase tracking-[0.1em] text-cream/60">Articles</p>
              <button type="button" className="min-h-11 rounded-bendjo-sm px-2 text-xs font-semibold text-cream underline underline-offset-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cream" onClick={handleClear}>Vider la sélection</button>
            </div>
            ...
            <div className="mt-3 border-t border-cream/20 pt-3">
              <div className="flex items-baseline justify-between gap-3">
                <p className="text-sm text-cream/75">Sous-total</p>
      ...
      <section id="selection" className="fixed inset-x-3 bottom-3 z-40 mx-auto max-w-md rounded-bendjo-md border border-cream/15 bg-ink p-3 text-cream shadow-bendjo-cart sm:inset-x-auto sm:bottom-6 sm:right-6 sm:w-[23rem] sm:max-w-none sm:p-4" aria-labelledby="cart-title">
        ...
        <div id="cart-details" className={`${isOpen ? "block" : "hidden"} sm:block`}>
          <div className="mt-3 border-t border-cream/20 pt-2">
            <div className="flex items-center justify-between gap-3">
              <p className="text-xs font-semibold uppercase tracking-[0.1em] text-cream/65">Articles</p>
              <button type="button" className="min-h-11 rounded-bendjo-sm px-2 text-xs font-semibold text-cream underline underline-offset-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cream" onClick={handleClear}>Vider</button>
            </div>
            <ul className="mt-1 max-h-[min(15rem,35svh)] overflow-y-auto pr-1">
              {state.items.map((item) => (
                <CartItem
