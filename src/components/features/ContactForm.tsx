...
function FormError({ id, children }: { id: string; children: string }) {
  return <p id={id} className="mt-2 inline-flex items-center gap-2 text-sm font-semibold text-leaf" role="alert"><WarningCircle size={17} weight="regular" aria-hidden="true" />{children}</p>;
}
  ...
  return (
    <form className="space-y-8" noValidate onSubmit={handleSubmit}>
      ...
      <div className="space-y-6 border-b border-leaf/15 pb-8">
        <div>
          <p className="type-label mb-4 text-leaf/75">Votre demande</p>
          <label className="block text-sm font-semibold text-leaf" htmlFor="contact-service-type"><RequiredLabel>Type de prestation</RequiredLabel></label>
        <select
        ...
        <div className="min-w-0">
          <label className="block text-sm font-semibold text-leaf" htmlFor="contact-name"><RequiredLabel>Nom</RequiredLabel></label>
          ...
          <div className="min-w-0">
          <label className="block text-sm font-semibold text-leaf" htmlFor="contact-email"><RequiredLabel>Adresse email</RequiredLabel></label>
        ...
        <div>
        <label className="block text-sm font-semibold text-leaf" htmlFor="contact-message"><RequiredLabel>Votre besoin</RequiredLabel></label>
      ...
      <details className="group border-b border-leaf/15 pb-8">
        ...
        <p className="mt-1 max-w-xl text-sm leading-6 text-leaf/75">Ces informations sont facultatives et peuvent aider BenDjo à préparer l’échange.</p>
        <div className="mt-6 grid min-w-0 gap-6 sm:grid-cols-2">
          <div className="min-w-0">
            <label className="block text-sm font-semibold text-leaf" htmlFor="contact-company">Entreprise</label>
          ...
          <div className="min-w-0">
            <label className="block text-sm font-semibold text-leaf" htmlFor="contact-phone">Téléphone</label>
          ...
          <div className="min-w-0">
            <label className="block text-sm font-semibold text-leaf" htmlFor="contact-location">Lieu</label>
          ...
          <div className="min-w-0">
            <label className="block text-sm font-semibold text-leaf" htmlFor="contact-date">Date envisagée</label>
            <input id="contact-date" className={fieldStyles} type="date" name="date" value={values.date} onChange={handleChange} />
          </div>
          <div className="min-w-0 sm:col-span-2">
            <label className="block text-sm font-semibold text-leaf" htmlFor="contact-guest-count">Nombre approximatif de personnes</label>
      ...
      {status === "ready" && (
        <p className="inline-flex items-center gap-2 text-sm leading-6 text-leaf" role="status" aria-live="polite"><Check size={18} weight="regular" aria-hidden="true" />Votre demande est prête. Autorisez l’ouverture de WhatsApp pour continuer.</p>
      )}
      {status === "openedWhatsApp" && (
        <p className="inline-flex items-center gap-2 text-sm leading-6 text-leaf" role="status" aria-live="polite"><Check size={18} weight="regular" aria-hidden="true" />Votre demande a été préparée dans WhatsApp. Vérifiez le message, puis envoyez-le vous-même pour le transmettre à BenDjo.</p>
      )}
    </form>
  );
}
